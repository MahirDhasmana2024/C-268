...
def water_effect(decode_array_to_img):
    water_effect_img = cv2.stylization(decode_array_to_img, sigma_s=60, sigma_r=0.9)
...
def hdr_effect(decode_array_to_img):
    hdr_effect_img = cv2.detailEnhance(decode_array_to_img, sigma_s=12, sigma_r=0.3)
